...
class Model(object):
    ...
    def build_fcn_net(self, inp):
        ...
        with tf.name_scope('Metrics'):
            if self.target_ph is not None:
                # TODO scalar, get_collection, AdamOptimizer.minimize, control_dependencies
    ...
    def train(self, sess, features, targets):
        loss, accuracy, aux_loss, _, probs = sess.run(
            [self.loss, self.accuracy, self.aux_loss, self.training_op, self.y_hat], feed_dict={
...
class Model_DMR(Model):
    def __init__(self, lr, global_step):
        ...
        self.dm_position_his_eb = tf.reshape(self.dm_position_his_eb, [tf.shape(self.mid)[0], -1,
                                                                       self.dm_position_his_eb.get_shape().as_list()[
                                                                           1]])  # B,T,E
        ...
        with tf.name_scope('u2i_net'):
            ...
            dm_item_biases = tf.get_variable('dm_item_biases', [cate_size], initializer=tf.zeros_initializer(),
                                             trainable=False)
            # Auxiliary Match Network
            self.aux_loss, dm_user_vector, scores = deep_match(self.item_his_eb, self.dm_position_his_eb, self.mask,
                                                               tf.cast(self.match_mask, tf.float32), self.cate_his,
                                                               main_embedding_size, dm_item_vectors, dm_item_biases,
                                                               cate_size)
            self.aux_loss *= 0.1
        ...
        with tf.name_scope('i2i_net'):
            att_outputs, alphas, scores_unnorm = dmr_fcn_attention(self.item_eb, self.item_his_eb, self.position_his_eb,
                                                                   self.mask)
